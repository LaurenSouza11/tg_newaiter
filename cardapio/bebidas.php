...
if (isset($_SESSION['LOGIN'])) {
	...
	$cabecalho = cabecalho('BEBIDAS', $_SESSION['LOCAL_SELECIONADO'], $userName , '');
	$rodape = rodape();
	...
	$query = "SELECT DISTINCT subgrupo, tipo_subgrupo
			  ...
			  WHERE ".$local." = 1 AND grupo = 1 ORDER BY tipo_subgrupo;";
	...
	if ($count_subgrupo > 0){
		...
		foreach($array_subgrupo as $tmp_subgrupo){
			...
			$query = "SELECT id, nome, descricao, preco, imagem
					  FROM cardapio
					  WHERE ".$local." = 1 AND grupo = 1 AND subgrupo = :subgrupo;";
				...
				foreach($result as $row) {
					...
					$pt2 = $pt2."<div class='col-6 col-md-4 col-lg-4 col-xl-3' style='margin-bottom: 25px;'>
									<div class='card altura-card'>
										<img class='card-img-top' style='height: 90px;' src='https://www.newaiter.com.br/newaiter/imagens/".$local."/cardapio/bebidas/".$imagem."' alt='Card image cap'>
										<div class='card-body'>
